...
import { DropdownChangeEvent, DropdownFilterEvent, DropdownFilterOptions, DropdownLazyLoadEvent } from './dropdown.interface.js';
        ...
        <div #container [attr.id]="id" [ngClass]="containerClass" (click)="onContainerClick($event)" [ngStyle]="style" [class]="styleClass!">
            ...
            <input
                *ngIf="editable"
                ...
                (keydown)="onKeyDown($any($event), false)"
                pAutoFocus
                [autofocus]="autofocus"
                [attr.aria-activedescendant]="focused ? focusedOptionId : undefined"
                (focus)="onInputFocus($event)"
...
export class Dropdown implements OnInit, AfterViewInit, AfterContentInit, AfterViewChecked, ControlValueAccessor {
    ...
    label = computed(() => {
        ...
        const selectedOptionIndex = options.findIndex((option: any) => this.isOptionValueEqualsModelValue(option));
    ...
    flatOptions(options: any[]) {
        return (options || []).reduce((result, option, index) => {
            result.push({ optionGroup: option, group: true, index });

            const optionGroupChildren = this.getOptionGroupChildren(option);

            optionGroupChildren && optionGroupChildren.forEach((o: any) => result.push(o));
    ...
    onOptionSelect(event: any, option: any, isHide = true, preventChange = false) {
    ...
    onOptionMouseEnter(event: any, index: any) {
        if (this.focusOnHover) {
            this.changeFocusedOptionIndex(event, index);
        }
    }

    updateModel(value: any, event?: any) {
        this.value = value;
    ...
    getOptionIndex(index: any, scrollerOptions: any) {
    ...
    getAriaPosInset(index: any) {
        return (
            (this.optionGroupLabel
                ? index -
                  this.visibleOptions()
                      .slice(0, index)
                      .filter((option: any) => this.isOptionGroup(option)).length
                : index) + 1
        );
    }

    get ariaSetSize() {
        return this.visibleOptions().filter((option: any) => !this.isOptionGroup(option)).length;
    }
    ...
    changeFocusedOptionIndex(event: any, index: any) {
    ...
    isValidSelectedOption(option: any) {
    ...
    findFirstOptionIndex() {
        return this.visibleOptions().findIndex((option: any) => this.isValidOption(option));
    }

    findSelectedOptionIndex() {
        return this.hasSelectedOption() ? this.visibleOptions().findIndex((option: any) => this.isValidSelectedOption(option)) : -1;
    }

    findNextOptionIndex(index: any) {
        const matchedOptionIndex =
            index < this.visibleOptions().length - 1
                ? this.visibleOptions()
                      .slice(index + 1)
                      .findIndex((option: any) => this.isValidOption(option))
                : -1;
        return matchedOptionIndex > -1 ? matchedOptionIndex + index + 1 : index;
    }

    findPrevOptionIndex(index: any) {
        const matchedOptionIndex = index > 0 ? ObjectUtils.findLastIndex(this.visibleOptions().slice(0, index), (option: any) => this.isValidOption(option)) : -1;

        return matchedOptionIndex > -1 ? matchedOptionIndex : index;
    }

    findLastOptionIndex() {
        return ObjectUtils.findLastIndex(this.visibleOptions(), (option: any) => this.isValidOption(option));
    }
    ...
    onEnterKey(event: any, pressedInInput = false) {
        if (!this.overlayVisible) {
            this.focusedOptionIndex.set(-1);
            this.onArrowDownKey(event);
        } else {
    ...
    onTabKey(event: any, pressedInInputText = false) {
        if (!pressedInInputText) {
    ...
    searchOptions(event: any, char: any) {
        ...
        if (this.focusedOptionIndex() !== -1) {
            optionIndex = this.visibleOptions()
                ...
                .findIndex((option: any) => this.isOptionMatched(option));
            optionIndex =
                optionIndex === -1
                    ? this.visibleOptions()
            ...
            optionIndex = this.visibleOptions().findIndex((option: any) => this.isOptionMatched(option));
        }
